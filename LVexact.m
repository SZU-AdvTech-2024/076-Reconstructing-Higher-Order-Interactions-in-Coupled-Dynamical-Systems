...
for iM = imin:istep:imax
    ...
    % fi(xi^m)
    TriangleList0=[4 1 6 0; 2 3 7 0];
    for n=0:nt-1
        pop = X(n+1,:);
        F(n+1,:) = LVeqComplete(n*dt,pop',zeros(N,N),R,K,TriangleList0);
    end

    A0=zeros(H,N); %reconstructed matrix, generated by OLS: Y_i = Phi_i * A_i
    AA=zeros(H,N); %"true" matrix, generated from A and TriangleList
    % Zero order method
    Phi = zeros(nt-2,H);
    for i=1:N
        %pairwise terms j=1:N-1
        Phi(:,1:N-1) = X(I1,i).*X(I1,[1:i-1 i+1:N]);  % \sum_{j=1}^{N} x_i * x_j 
        ...
        for ii1=[1:i-1 i+1:N]
            for jj1=[1:i-1 i+1:N]
                if jj1>ii1
                    vtemp(:,itemp)=X(I1,i).*X(I1,ii1).*X(I1,jj1);  % \sum_{j=1}^{N} \sum_{k=j+1}^{N} x_i * x_j *x_k
