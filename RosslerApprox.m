...
parfor iMindice = 1:length(iMvalori)
    ...
    A6=zeros(H,N); %reconstructed matrix with LS, fourth order approx
    AA=zeros(H,N); %vera
    EdgeList0=[1 1; 2 2];
    ...
    for i=1:N
        ...
        Zi6 = signal_lasso_2(Yi, Phi, H, 0.001, 0.001)
    ...
    err6(iMindice,1)=norm(AA-A6,"fro")/normA;
end

save results.mat

figure,loglog(irange,err4,irange,err5, irange,err6);
xlabel('M/H')
ylabel('E')
legend('OLS', 'NNLS', 'LS')