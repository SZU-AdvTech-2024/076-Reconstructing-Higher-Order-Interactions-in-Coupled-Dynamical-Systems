...
%%% and S. Boccaletti, Physical Review Research 3, 043210 (2021)
    ...
    while iters < num_iters && res > 1e-11
        ...
        for j = 1:n  % for each coordinator
            y_pred = X*w; 
            y_diff = (y - y_pred);
            ...
            if dot(X_j', X_j) == 0  % W_j is zero vector
                z = 0;
            else
...
function out = threshold_signal(z, X_j, alpha, alpha_2)
    if dot(X_j', X_j) == 0
        ...
        delta_1 = (alpha + alpha_2) / dot(X_j', X_j);  % \delta^*_1k in eq(B2)
        delta_2 = (alpha - alpha_2) / dot(X_j', X_j);  % \delta^*_2k in eq(B2)
    end

    % corresponding to eq(B4)
    if z <= 0
